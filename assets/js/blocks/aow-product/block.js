...
/**
 * Component to handle edit mode of "Aow Product".
 */
const AowProduct = ( {
	attributes,
	debouncedSpeak,
	...
	const renderApiError = () => (
		<ErrorPlaceholder
			className="wc-block-aow-product-error"
			error={ error }
			isLoading={ isLoading }
			onRetry={ getProduct }
		/>
	...
	const renderEditMode = () => {
		const onDone = () => {
			...
			debouncedSpeak(
				__(
					'Showing Aow Product block preview.',
		...
		return (
			<Fragment>
				...
				<Placeholder
					...
					className="wc-block-aow-product"
				>
					{ __(
						'Visually highlight a product or variation and encourage prompt action',
						'woo-gutenberg-products-block'
					) }
					<div className="wc-block-aow-product__selection">
						<ProductControl
	...
	const renderProduct = () => {
		...
		return (
			<ResizableBox
				...
				style={ style }
			>
				<div className="wc-block-aow-product__wrapper">
					<h2
						className="wc-block-aow-product__title"
					...
					{ ! isEmpty( product.variation ) && (
						<h3
							className="wc-block-aow-product__variation"
							dangerouslySetInnerHTML={ {
					...
					{ showDesc && (
						<div
							className="wc-block-aow-product__description"
							dangerouslySetInnerHTML={ {
					...
					{ showPrice && (
						<div
							className="wc-block-aow-product__price"
							dangerouslySetInnerHTML={ {
								__html: product.price_html,
							} }
						/>
					) }
					<div className="wc-block-aow-product__link">
						{ renderButton() }
					</div>
				</div>
			</ResizableBox>
	...
	const renderNoProduct = () => (
		<Placeholder
			className="wc-block-aow-product"
			icon="star-filled"
			label={ __( 'Aow Product', 'woo-gutenberg-products-block' ) }
		>
			{ isLoading ? (
				<Spinner />
			) : (
