 ...
 * AowProduct class.
 */
class AowProduct extends AbstractDynamicBlock {
	/**
	 * Block name.
	 *
	 * @var string
	 */
	protected $block_name = 'aow-product';
	...
	public function render( $attributes = array(), $content = '' ) {
		...
		if ( ! $attributes['height'] ) {
			$attributes['height'] = wc_get_theme_support( 'aow_block::default_height', 500 );
		}

		$title = sprintf(
			'<h2 class="wc-block-aow-product__title">%s</h2>',
			wp_kses_post( $product->get_title() )
		);

		if ( $product->is_type( 'variation' ) ) {
			$title .= sprintf(
				'<h3 class="wc-block-aow-product__variation">%s</h3>',
				wc_get_formatted_variation( $product, true, true, false )
			);
		}

		$desc_str = sprintf(
			'<div class="wc-block-aow-product__description">%s</div>',
		...
		$price_str = sprintf(
			'<div class="wc-block-aow-product__price">%s</div>',
		...
		$output .= '<div class="wc-block-aow-product__link">' . $content . '</div>';
		$output .= '</div>';
